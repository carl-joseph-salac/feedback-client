        ...
        <div class="container mb-3 mt-0">
            <div class="container rounded border">
                <div class="px-4">
                    ...
                    <form action="{{ route(session('sqd1Edit', 'sqd2')) }}" method="post">
                        @csrf
                        @method('post')
                        <p>
                            ...
                            <div class="rating-wrap">
                                <div class="center text-center">
                                    <fieldset class="rating">
                                        <input type="radio" id="star5" name="sqd1"
                                        value="{{$sqd->choices5}}" {{ session('sqd1') == $sqd->choices5 ? 'checked' : '' }}/>
                                        <label for="star5" class="full" title="Awesome"></label>

                                        <input type="radio" id="star4" name="sqd1"
                                        value="{{$sqd->choices4}}" {{ session('sqd1') == $sqd->choices4 ? 'checked' : '' }}/>
                                        <label for="star4" class="full"></label>

                                        <input type="radio" id="star3" name="sqd1"
                                        value="{{$sqd->choices3}}" {{ session('sqd1') == $sqd->choices3 ? 'checked' : '' }}/>
                                        <label for="star3" class="full"></label>

                                        <input type="radio" id="star2" name="sqd1"
                                        value="{{$sqd->choices2}}" {{ session('sqd1') == $sqd->choices2 ? 'checked' : '' }}/>
